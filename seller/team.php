...
function team_order_list($ru_id, $team_id = 0)
{
	...
	while ($row = $GLOBALS['db']->fetchRow($res)) {
		...
		$arr['status'] = $_LANG['os'][$arr['order_status']] . ',' . $_LANG['ps'][$arr['pay_status']] . ',' . $_LANG['ss'][$arr['shipping_status']];
		$list[] = $arr;
	}
...
if ($_REQUEST['act'] == 'list') {
	...
	if ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit') {
		...
		$team_list = team_cat_list(0, $goods['tc_id']);
		...
		if (0 < $id) {
			...
			$links = array(
				array('href' => 'team.php?act=list&' . list_link_postfix(), 'text' => '返回拼团列表')
				);
			...
			if (1 <= $res['num']) {
				$links = array(
					array('href' => 'team.php?act=add', 'text' => '继续添加拼团商品')
					);
				sys_msg('该拼团商品活动结束之前，不可添加新的活动', 0, $links);
			}

			$db->autoExecute($ecs->table('team_goods'), $goods, 'INSERT');
			$links = array(
				array('href' => 'team.php?act=add', 'text' => '继续添加拼团商品'),
				array('href' => 'team.php?act=list', 'text' => '返回拼团商品列表')
				);
			sys_msg('拼团商品添加成功', 0, $links);
		}
	}
		...
		if (isset($_POST['checkboxes'])) {
			...
			$links[] = array('text' => '返回拼团商品列表', 'href' => 'team.php?act=list');
			sys_msg(sprintf('拼团商品删除成功', $del_count), 0, $links);
		}
		else {
...
if ($_REQUEST['act'] == 'team_info') {
	...
	if (isset($_POST['checkboxes'])) {
		...
		$links[] = array('text' => '返回拼团商品列表', 'href' => 'team.php?act=team_info');
		sys_msg(sprintf('拼团商品删除成功', $del_count), 0, $links);
	}
	else {
