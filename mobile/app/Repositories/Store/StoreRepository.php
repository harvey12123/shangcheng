...
class StoreRepository
{
	public function all($uid, $page, $size)
	{
		...
		foreach ($store as $key => $val) {
			...
			$store[$key]['goods'] = \App\Models\Goods::select('goods_id', 'goods_sn', 'brand_id', 'goods_name', 'goods_thumb', 'shop_price', 'promote_price', 'cat_id', 'market_price', 'goods_number')->where('user_id', $val['user_id'])->where('is_on_sale', 1)->where('review_status', '>', 2)->where('is_alone_sale', 1)->limit(4)->orderBy('sort_order', 'ASC')->get()->toArray();
	...
	public function goods($id, $page, $per_page, $cate_key, $sort, $order, $cat_id)
	{
		...
		if (0 < $cat_id) {
			$goods = \App\Models\Goods::select('goods_id', 'goods_sn', 'brand_id', 'goods_name', 'goods_thumb', 'shop_price', 'promote_price', 'cat_id', 'market_price', 'goods_number')->where('user_id', $id)->where('is_on_sale', 1)->where('review_status', '>', 2)->where('is_alone_sale', 1)->where($cate_key, '1')->where('user_cat', $cat_id)->offset($current)->limit($per_page)->orderBy($sort, $order)->get()->toArray();
		}
		else {
			$goods = \App\Models\Goods::select('goods_id', 'goods_sn', 'brand_id', 'goods_name', 'goods_thumb', 'shop_price', 'promote_price', 'cat_id', 'market_price', 'goods_number')->where('user_id', $id)->where('is_on_sale', 1)->where('review_status', '>', 2)->where('is_alone_sale', 1)->where($cate_key, '1')->offset($current)->limit($per_page)->orderBy($sort, $order)->get()->toArray();
		}
