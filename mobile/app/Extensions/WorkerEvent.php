...
class WorkerEvent
{
	...
	public function __construct()
	{
		...
		if (!isset($c['listen_route']) || empty($c['listen_route'])) {
			exit(' listen_route need to be configured ');
		}

		if (!isset($c['root_path']) || empty($c['root_path'])) {
			exit(' root_path need to be configured ');
		}

		if (!isset($c['port']) || empty($c['port'])) {
			exit(' port need to be configured ');
		}
	...
	{
		\App\Models\ImService::whereRaw('1=1')->update(array('chat_status' => 0));
	}
	...
	public function closeolddialog()
	{
		...
		return $this->db(array('close_all_dialog' => 'close_all_dialog'), $url);
	}

	public function setpem($cert, $pk)
	{
		$this->context = array(
			'ssl' => array('local_cert' => $cert, 'local_pk' => $pk, 'verify_peer' => false)
			);
	}
}
