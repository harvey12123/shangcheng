...
class IndexController extends FrontendController
{
    ...
    public function actionTeamlist()
    {
        ...
        if (IS_AJAX) {
            ...
            $arr = [];
            $sql = 'SELECT g.goods_id,g.user_id, g.goods_name, g.shop_price, g.goods_name_style, g.comments_number, g.sales_volume, g.market_price, g.goods_thumb , g.goods_img, tg.id, tg.team_price, tg.team_num FROM ' . $this->ecs->table('team_goods') . 'AS tg LEFT JOIN ' . $this->ecs->table('goods') . ' AS g ON tg.goods_id = g.goods_id ' .
                "WHERE $where order by tg.id desc";
    ...
    public function actionvirtualorder()
    {
        ...
        if ($is_open['value'] == 1) {
            ...
            $sql = 'SELECT user_name, user_id FROM ' . $this->ecs->table('users') . " WHERE user_id <> " . $_SESSION['user_id'] . " and nick_name !='' ORDER BY rand() LIMIT 1";
