...
class GoodsController extends FrontendController
{
    ...
    public function actionIndex()
    {
        ...
        $subscribe = dao('wechat_user')->where(['ect_uid' => $this->user_id])->getField('subscribe');
        $this->assign('subscribe', $subscribe);
        
        if ($this->team_id) {
            $team_info = team_is_failure($this->team_id);
            if ($team_info['is_team'] != 1) {
                show_message('该拼团活动已结束，去查看新的活动吧', '查看新的活动', url('team/index/index'), 'success');
            }
        }
        ...
        $team = $this->db->table('team_goods')->field('id,team_price,team_num,limit_num,astrict_num,is_audit,is_team,team_desc')->where(['goods_id' => $this->goods_id, 'is_team' => '1'])->find();
        $goods['t_id'] = $team['id'];
        ...
        if ($team['is_team'] == 0 || $team['is_audit'] != 2) {
            show_message('该拼团活动已结束，去查看新的活动吧', '查看新的活动', url('team/index/index'), 'success');
        }
        ...
        $share_data = [
            'title' => '我正在'.$goods['team_price'].'元拼' . $goods['goods_name'],
            'desc' => $goods['team_desc'] ? $goods['team_desc'] : $goods['goods_brief'],
            'link' => '',
            'img' => $goods['goods_img'],
        ];
    ...
    public function actionTeambuy()
    {
        ...
        if ($team_id > 0) {
            $team_log = dao('team_log')->where(["team_id" => $team_id])->find();
            if ($team_log['status'] == 1) {
                show_message('该团已经完成，请选择其他团去参团', '', '', 'error');
            }
        }
