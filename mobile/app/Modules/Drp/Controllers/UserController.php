...
class UserController extends FrontendController
{
    ...
    public function get_drp_goods($type = '')
    {
        ...
        $sql = "select g.goods_id,g.dis_commission, g.user_id, g.goods_name, g.goods_brief, g.goods_thumb , g.goods_img, IFNULL(mp.user_price, IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) * '$_SESSION[discount]') AS shop_price from {pre}goods AS g $leftJoin LEFT JOIN  {pre}member_price  AS mp
        ON mp.goods_id = g.goods_id AND mp.user_rank = $_SESSION[user_rank] where is_on_sale = 1  AND dis_commission>0 and is_distribution=1 AND is_alone_sale = 1 AND is_delete = 0 $where ORDER BY goods_id desc limit 0,5 ";
        $res = $GLOBALS['db']->getAll($sql);
        $goods_list = [];
        ...
        if ($generate) {
            ...
            if ($qrcodeInfo) {
                ...
                if (!file_exists($avatar)) {
                    ...
                    $img->open($avatar_open)->thumb(100, 100, Image::IMAGE_THUMB_FILLED)->save($avatar);
                }
    ...
    public function actionTeam()
    {
        if (IS_AJAX) {
            ...
            foreach ($next as $key => $value) {
                ...
                $user_nick = get_wechat_user_info($value['user_id']);
                $next[$key]['headimgurl'] = get_image_path($user_nick['user_picture'], '', elixir('img/user_default.png'));
                $next[$key]['user_name'] = $user_nick['nick_name'];
            }           
    ...
    public function actionOfflineUser()
    {
        if (IS_AJAX) {
            $uid = I('user_id') ? I('user_id') : $this->user_id;
            $page = I('page', 1);
            $size = 10;
            $sql = "SELECT user_id,reg_time FROM {pre}users 
                    WHERE  drp_parent_id='$uid'
            ...
            foreach ($list as $key => $value) {
                ...
                $user_nick = get_wechat_user_info($value['user_id']);
                $list[$key]['headimgurl'] = get_image_path($user_nick['user_picture'], '', elixir('img/user_default.png'));
                $list[$key]['user_name'] = $user_nick['nick_name'];
                $list[$key]['reg_time'] = local_date($GLOBALS['_CFG']['time_format'], $value['reg_time']);
            } 
            exit(json_encode(['info' => $list, 'totalPage' => ceil($total / $size)]));
        }
        $this->assign('page_title', L('my_offline_user'));
        $this->assign('next_id', I('user_id', ''));
        $this->display();
    }
    ...
    public function actionTeamDetail()
    {
        ...
        $user_nick = get_wechat_user_info($info['user_id']);
        $info['headimgurl'] = get_image_path($user_nick['user_picture'], '', elixir('img/user_default.png'));
        $info['user_name'] = $user_nick['nick_name'];
        $this->assign('info', $info);
        ...
        if (!$all) {
            ...
            if ($all) {
                foreach ($all as $key => $val) {
                    ...
                    $user_nick = get_wechat_user_info($all[$key]['user_id']);
                    $all[$key]['headimgurl'] = get_image_path($user_nick['user_picture'], '', elixir('img/user_default.png'));
                    $all[$key]['user_name'] = $user_nick['nick_name'];
                    if ($key === 0) {
