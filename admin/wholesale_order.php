...
elseif ($_REQUEST['act'] == 'get_goods_info')
{
    ...
    $goods_list = array();
    $goods_attr = array();
    $sql = "SELECT o.*, g.goods_thumb, g.goods_sn, g.brand_id, g.user_id AS ru_id, w.act_id, w.goods_number AS storage, g.model_inventory, o.goods_attr, oi.order_sn " .
            "FROM " . $ecs->table('wholesale_order_goods') . " AS o ".
            "LEFT JOIN " . $ecs->table('goods') . " AS g ON o.goods_id = g.goods_id " .
            "LEFT JOIN " . $ecs->table('wholesale') . " AS w ON w.goods_id = g.goods_id " .
...
{	
	...
	if(isset($_POST['remove'])){
		foreach ($order_id_list as $id_order)
		{
	        ...
	        $order_ids = $db->getOne("SELECT order_id FROM " . $ecs->table('wholesale_order_info') . " WHERE order_sn= " . $id_order);

			
			$db->query("DELETE FROM ".$ecs->table('wholesale_order_info'). " WHERE order_id = '$order_ids'");
			$db->query("DELETE FROM ".$ecs->table('wholesale_order_goods'). " WHERE order_id = '$order_ids'");
			$db->query("DELETE FROM ".$ecs->table('wholesale_order_action'). " WHERE order_id = '$order_ids'");
