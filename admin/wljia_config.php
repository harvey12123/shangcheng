...
if ($_REQUEST['act'] == 'list') {
	$filter = array();
	$smarty->assign('ur_here', $_LANG['12_wljia_app_config']);
	$shop_app_icon = wljia_config('shop_app_icon') ? ROOT_PATH . wljia_config('shop_app_icon') : '';
	$smarty->assign('shop_app_icon', wljia_config('shop_app_icon'));
	$smarty->assign('shop_app_description', wljia_config('shop_app_description'));
	$smarty->assign('bonus_readme_url', wljia_config('bonus_readme_url'));
	$smarty->assign('mobile_feedback_autoreply', wljia_config('mobile_feedback_autoreply'));
	$smarty->assign('mobile_shopkeeper_urlscheme', wljia_config('mobile_shopkeeper_urlscheme'));
	$smarty->assign('shop_pc_url', wljia_config('shop_pc_url'));
	$smarty->assign('mobile_share_link', wljia_config('mobile_share_link'));
	$time = gmtime();
	...
	$bonus_id = wljia_config('mobile_signup_reward');
	...
	$smarty->assign('mobile_signup_reward_notice', wljia_config('mobile_signup_reward_notice'));
	$smarty->assign('mobile_iphone_qr_code', wljia_config('mobile_iphone_qr_code'));
	$smarty->assign('shop_iphone_download', wljia_config('shop_iphone_download'));
	$smarty->assign('mobile_android_qr_code', wljia_config('mobile_android_qr_code'));
	$smarty->assign('shop_android_download', wljia_config('shop_android_download'));
	$smarty->assign('mobile_ipad_qr_code', wljia_config('mobile_ipad_qr_code'));
	$smarty->assign('shop_ipad_download', wljia_config('shop_ipad_download'));
	...
	$launch_ad_id = wljia_config('mobile_launch_adsense');
	...
	$ads_id = wljia_config('mobile_home_adsense_group');
	$where = '';
	...
	$regions_id = wljia_config('mobile_recommend_city');
	$where1 = '';
	...
	$topic_ad_id = wljia_config('mobile_topic_adsense');
	...
	$smarty->assign('mobile_phone_login_fgcolor', wljia_config('mobile_phone_login_fgcolor'));
	$smarty->assign('mobile_phone_login_bgcolor', wljia_config('mobile_phone_login_bgcolor'));
	$smarty->assign('mobile_phone_login_bgimage', wljia_config('mobile_phone_login_bgimage'));
	$smarty->assign('mobile_pad_login_fgcolor', wljia_config('mobile_pad_login_fgcolor'));
	$smarty->assign('mobile_pad_login_bgcolor', wljia_config('mobile_pad_login_bgcolor'));
	$smarty->assign('mobile_pad_login_bgimage', wljia_config('mobile_pad_login_bgimage'));
	$smarty->assign('mobile_recommend_city', wljia_config('mobile_recommend_city'));
	$smarty->assign('form_action', 'update');
	assign_query_info();
	$smarty->display('wljia_config.dwt');
}
...
if ($_REQUEST['act'] == 'update') {
	...
	if (isset($_FILES['shop_app_icon']['error']) && $_FILES['shop_app_icon']['error'] == 0 || !isset($_FILES['shop_app_icon']['error']) && isset($_FILES['shop_app_icon']['tmp_name']) && $_FILES['shop_app_icon']['tmp_name'] != 'none') {
		$img_up_info = basename($image->upload_image($_FILES['shop_app_icon'], 'assets/wlmoban_sc'));
		$code = wljia_config('shop_app_icon');
		if ($code && $code != DATA_DIR . '/assets/wlmoban_sc/' . $img_up_info) {
			@unlink('../' . $code);
		}

		get_oss_add_file(array(DATA_DIR . '/assets/wlmoban_sc/' . $img_up_info));
		$shop_app_icon_img_src = DATA_DIR . '/assets/wlmoban_sc/' . $img_up_info;
	}
	...
	if (isset($_FILES['mobile_iphone_qr_code']['error']) && $_FILES['mobile_iphone_qr_code']['error'] == 0 || !isset($_FILES['mobile_iphone_qr_code']['error']) && isset($_FILES['mobile_iphone_qr_code']['tmp_name']) && $_FILES['mobile_iphone_qr_code']['tmp_name'] != 'none') {
		...
		$code = wljia_config('mobile_iphone_qr_code');
	...
	if (isset($_FILES['mobile_android_qr_code']['error']) && $_FILES['mobile_android_qr_code']['error'] == 0 || !isset($_FILES['mobile_android_qr_code']['error']) && isset($_FILES['mobile_android_qr_code']['tmp_name']) && $_FILES['mobile_android_qr_code']['tmp_name'] != 'none') {
		...
		$code = wljia_config('mobile_android_qr_code');
		if ($code && $code != DATA_DIR . '/assets/' . $img_up_info) {
			@unlink('../' . $code);
		}
	...
	if (isset($_FILES['mobile_ipad_qr_code']['error']) && $_FILES['mobile_ipad_qr_code']['error'] == 0 || !isset($_FILES['mobile_ipad_qr_code']['error']) && isset($_FILES['mobile_ipad_qr_code']['tmp_name']) && $_FILES['mobile_ipad_qr_code']['tmp_name'] != 'none') {
		...
		$code = wljia_config('mobile_ipad_qr_code');
		if ($code && $code != DATA_DIR . '/assets/' . $img_up_info) {
			@unlink('../' . $code);
		}
	...
	if (isset($_FILES['mobile_phone_login_bgimage']['error']) && $_FILES['mobile_phone_login_bgimage']['error'] == 0 || !isset($_FILES['mobile_phone_login_bgimage']['error']) && isset($_FILES['mobile_phone_login_bgimage']['tmp_name']) && $_FILES['mobile_phone_login_bgimage']['tmp_name'] != 'none') {
		...
		$code = wljia_config('mobile_phone_login_bgimage');
		if ($code && $code != DATA_DIR . '/assets/' . $img_up_info) {
			@unlink('../' . $code);
		}
	...
	if (isset($_FILES['mobile_pad_login_bgimage']['error']) && $_FILES['mobile_pad_login_bgimage']['error'] == 0 || !isset($_FILES['mobile_pad_login_bgimage']['error']) && isset($_FILES['mobile_pad_login_bgimage']['tmp_name']) && $_FILES['mobile_pad_login_bgimage']['tmp_name'] != 'none') {
		...
		$code = wljia_config('mobile_pad_login_bgimage');
		if ($code && $code != DATA_DIR . '/assets/' . $img_up_info) {
			@unlink('../' . $code);
		}
	...
	$link[0]['href'] = 'wljia_config.php?act=list';
	...
	$article_str .= "</ul>\r\n            <input name=\"bonus_readme_url\" type=\"hidden\" value=\"" . wljia_config('$bonus_readme_url') . '" id="bonus_readme_url">';
